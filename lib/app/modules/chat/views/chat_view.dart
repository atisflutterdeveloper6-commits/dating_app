      ...
      builder: (context) => SafeArea(
        child: Wrap(
          children: [
            ...
            ListTile(
              leading: const Icon(Icons.insert_drive_file, color: Color(0xffFF6A00)),
...
void _startCall(bool isVideoCall) async {
  ...
  try {
    ...
    // ✅ send() ne bool return kiya — false matlab invitation deliver nahi hua
    if (!success) {
      print('⚠️ Call invitation returned false for $otherUserId');
      CustomToast.error('$otherUserName is currently unavailable. They may not have opened the app yet.');
      return; // ✅ Call message save mat karo agar deliver hi nahi hua
    }

    // ✅ Sirf successful invitation pe hi call-log message save karo
    if (chatRoomId != null) {
    ...
    // ✅ ZegoCloud ka specific error code — user Zego se kabhi register/login nahi hua ya offline hai
...
Future<bool> _checkCallPermissions(bool isVideoCall) async {
  ...
  final camGranted = !isVideoCall || (statuses[Permission.camera]?.isGranted ?? false);
...
// ✅ Doosre user ka typing status suno
  void _listenTypingStatus() {
    ...
    _chatService.getConversationStream(chatRoomId!).listen(
      (snapshot) {
        try {
 ...
 void _loadUserStatus() {
    ...
    _chatService.getUserStatus(otherUserId!).listen(
      (snapshot) {
        try {
          if (snapshot.exists && mounted) {
            final data = snapshot.data() as Map<String, dynamic>?;
            ...
            // ✅ DEBUG PRINTS
            print('🟢 [ChatView] otherUserId: $otherUserId');
            print('🟢 [ChatView] raw online flag: $isOnlineFlag');
            print('🟢 [ChatView] lastSeen: $lastSeen');
            
            bool isOnline = false;
            if (isOnlineFlag && lastSeen != null) {
              final diff = DateTime.now().difference(lastSeen.toDate());
              print('🟢 [ChatView] diff in seconds: ${diff.inSeconds}');
              isOnline = diff.inSeconds < 60;
            } else {
              print('🟢 [ChatView] Skipped diff check — flag: $isOnlineFlag, lastSeen null: ${lastSeen == null}');
            }
            
            print('🟢 [ChatView] FINAL calculated isOnline: $isOnline');
...
Future<void> _sendMessage({String? imagePath, String? filePath, String? fileName}) async {
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Text(
              'Today',
              style: gf.GoogleFonts.poppins(fontSize: 12.sp, color: Colors.black87),
            ),
          ),
      ...
      actions: [
        Padding(
          ...
          child: Row(
            children: [
             GestureDetector(
  onTap: () => _startCall(true), // ✅ video call
  child: Container(
...
GestureDetector(
  ...
  child: Container(
    ...
    child: SvgPicture.asset("assets/icons/Call.svg", width: 16.w, height: 16.h),
  ),
),
            ],
          ),
    ...
    builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
      ...
      return NotificationListener<ScrollNotification>(
        ...
        child: ListView.builder(
          ...
          reverse: true, // ✅ KEY FIX: Reverse the list to show newest at bottom
          ...
          itemBuilder: (context, index) {
            try {
              ...
              final hasImage = msg['imageUrl'] != null && msg['imageUrl'].toString().isNotEmpty;
               final hasFile = msg['fileUrl'] != null && msg['fileUrl'].toString().isNotEmpty; // ✅ add
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Text(
              message,
              style: gf.GoogleFonts.poppins(fontSize: 12.sp, color: Colors.white),
            ),
          ),
          SizedBox(height: 3.h),
          Text(time, style: gf.GoogleFonts.poppins(fontSize: 10.sp, color: Colors.grey)),
        ],
      ),
    );
  }
  // ✅ Call Log Message
  Widget _callMessage({required String callType, required String time, required bool isSender}) {
    return Align(
        ...
        children: [
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isSender ? const Color(0xffFF6A00) : const Color(0xffA89D99),
                  ...
                  child: Center(
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Icon(Icons.download, color: Colors.white, size: 24.sp),
                      ),
                    ),
                  ),
                ),
